import Link from "next/link";
import { Phone, Mail, MapPin } from "lucide-react";

export default function Footer() {
  return (
    <footer className="bg-maroon text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Main Footer */}
        <div className="py-12 md:py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          {/* Brand */}
          <div className="lg:col-span-1">
            <h3 className="text-3xl font-heading font-bold text-saffron mb-3">
              Swaada
            </h3>
            <p className="text-white/80 text-sm leading-relaxed mb-4">
              Every Occasion, Perfectly Served. Premium catering services
              across Karnataka for memorable celebrations.
            </p>
            <div className="flex items-center gap-2 text-white/60 text-sm">
              <MapPin size={16} className="text-saffron flex-shrink-0" />
              <span>Serving all across Karnataka</span>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="text-lg font-heading font-semibold mb-4 text-saffron">
              Quick Links
            </h4>
            <ul className="space-y-2.5">
              {[
                { href: "/", label: "Home" },
                { href: "/menu", label: "Our Menu" },
                { href: "/about", label: "About Us" },
                { href: "/contact", label: "Contact" },
                { href: "/book", label: "Book Now" },
              ].map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-white/70 hover:text-saffron text-sm transition-colors duration-200"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Services */}
          <div>
            <h4 className="text-lg font-heading font-semibold mb-4 text-saffron">
              We Cater For
            </h4>
            <ul className="space-y-2.5 text-white/70 text-sm">
              {[
                "Weddings",
                "Birthdays",
                "Corporate Events",
                "Housewarmings",
                "Religious Functions",
                "Festivals",
              ].map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h4 className="text-lg font-heading font-semibold mb-4 text-saffron">
              Get in Touch
            </h4>
            <div className="space-y-3">
              <a
                href="tel:7892861158"
                className="flex items-center gap-3 text-white/70 hover:text-saffron transition-colors text-sm"
              >
                <Phone size={16} className="text-saffron flex-shrink-0" />
                +91 7892861158
              </a>
              <a
                href="tel:8105758067"
                className="flex items-center gap-3 text-white/70 hover:text-saffron transition-colors text-sm"
              >
                <Phone size={16} className="text-saffron flex-shrink-0" />
                +91 8105758067
              </a>
              <a
                href="mailto:hello.swaadacaterings@gmail.com"
                className="flex items-center gap-3 text-white/70 hover:text-saffron transition-colors text-sm"
              >
                <Mail size={16} className="text-saffron flex-shrink-0" />
                hello.swaadacaterings@gmail.com
              </a>
              <a
                href="https://wa.me/917892861158"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 mt-3 px-5 py-2.5 bg-whatsapp text-white rounded-full text-sm font-medium hover:bg-whatsapp/90 transition-all hover:scale-105"
              >
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                Chat on WhatsApp
              </a>
            </div>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="border-t border-white/10 py-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-white/50 text-xs">
            © {new Date().getFullYear()} Swaada Caterings & Services. All rights
            reserved.
          </p>
          <p className="text-white/50 text-xs">
            Made with ❤️ in Karnataka
          </p>
        </div>
      </div>
    </footer>
  );
}
